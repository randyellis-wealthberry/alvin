...
function ContactFormModal({
  ...
  const [notifyByEmail, setNotifyByEmail] = useState(contact?.notifyByEmail ?? true)
      ...
      <div className="relative w-full max-w-md animate-reveal">
        <GlassCard className="p-6" glow="violet">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Priority */}
            <div>
              <label className="block text-sm text-white/60 mb-3">Alert Priority</label>
            ...
            {/* Error */}
            {error && (
              <div className="px-4 py-3 rounded-xl bg-rose-500/20 text-rose-300 text-sm">
                {error.message}
              </div>
            )}
            ...
            {/* Success */}
            {showSuccess && (
              <div className="px-4 py-3 rounded-xl bg-emerald-500/20 text-emerald-300 text-sm flex items-center gap-2">
      ...
      <div className="relative w-full max-w-sm animate-reveal">
        <GlassCard className="p-6" glow="rose">
          ...
          <p className="text-white/60 text-center text-sm mb-6">
            Are you sure you want to remove <strong className="text-white">{contact.name}</strong> from your contacts?
          </p>
          <div className="flex gap-3">
            ...
            <button
              onClick={onConfirm}
              disabled={isPending}
              className="flex-1 py-3 rounded-xl bg-rose-500 text-white font-medium hover:bg-rose-400 transition-colors disabled:opacity-50"
            >
              {isPending ? <Loader2 className="w-5 h-5 mx-auto animate-spin" /> : "Delete"}
            </button>
          </div>
        </GlassCard>
      </div>
...
function ContactCard({
  ...
  return (
    ...
    >
      <GlassCard className="p-5 group hover:border-white/10 transition-colors">
        {/* Priority Gradient Overlay */}
        <div
          className={cn(
            "absolute inset-0 bg-gradient-to-br opacity-50 pointer-events-none",
            priorityColors[contact.priority as keyof typeof priorityColors] || priorityColors[4]
          )}
        />
        ...
        <div className="relative">
          ...
          <div className="flex items-center gap-4 text-xs text-white/40 mb-4">
            <span className="flex items-center gap-1">
              <Bell className={cn("w-3 h-3", contact.notifyByEmail && "text-emerald-400")} />
          ...
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={onEdit}
              className="flex-1 py-2 rounded-lg bg-white/5 text-white/60 text-sm font-medium hover:bg-white/10 hover:text-white transition-colors flex items-center justify-center gap-2"
            >
            ...
            <button
              onClick={onDelete}
              className="flex-1 py-2 rounded-lg bg-white/5 text-white/60 text-sm font-medium hover:bg-rose-500/20 hover:text-rose-400 transition-colors flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-[#1a0533] via-[#2e026d] to-[#15162c]">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-violet-500/15 rounded-full blur-3xl animate-blob" />
        <div className="absolute top-1/2 -right-40 w-80 h-80 bg-blue-500/10 rounded-full blur-3xl animate-blob animation-delay-2000" />
        <div className="absolute bottom-20 left-1/4 w-72 h-72 bg-cyan-500/10 rounded-full blur-3xl animate-blob animation-delay-4000" />
      </div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all"
              >
            ...
            >
              <GlassCard className="p-10 text-center" glow="violet">
                ...
                <p className="text-white/60 mb-6 max-w-sm mx-auto">
                  Add trusted contacts who will be notified if you miss a wellness check-in
                </p>
                <button
                  onClick={() => setShowAddModal(true)}
                  className={cn(
                ...
                <div className="space-y-3">
                  {PRIORITY_INFO.map((p) => (
                    <div key={p.level} className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-white">{p.label} to be notified</p>
